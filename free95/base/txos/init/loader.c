...
Abstract:
    ...
    This module implements the code to load PE Executable files.
...
/* Check if provided image is a valid Portable Executable */
WINBOOL IsValidPE(const LPVOID lpImage)
{
    PIMAGE_DOS_HEADER lpImageDOSHeader = (PIMAGE_DOS_HEADER)lpImage;
	PIMAGE_NT_HEADERS lpImageNTHeader = (PIMAGE_NT_HEADERS)((unsigned long)lpImageDOSHeader + lpImageDOSHeader->e_lfanew);
...
/* Check if provided image is a DLL */
WINBOOL IsDLL(const LPVOID hDLLData)
{
    PIMAGE_DOS_HEADER lpImageDOSHeader = (PIMAGE_DOS_HEADER)(hDLLData);
	PIMAGE_NT_HEADERS lpImageNtHeader = (PIMAGE_NT_HEADERS)((unsigned long)hDLLData + lpImageDOSHeader->e_lfanew);
...
/* Check if provided image has same matching architecture as host machine*/
...
{
    PIMAGE_DOS_HEADER lpImageDOSHeader = (PIMAGE_DOS_HEADER)lpImage;
	PIMAGE_NT_HEADERS lpImageNTHeader = (PIMAGE_NT_HEADERS)((unsigned long)lpImage + lpImageDOSHeader->e_lfanew);
	const unsigned long dVirtualAddress = lpImageNTHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS].VirtualAddress;
...
LPVOID LdrLoadPe(const LPSTR path)
{
	...
	if (fd)
	{
		char pBufImageFile[307200];
		fread(pBufImageFile, 307199, 1, fd);

        PPEImageFileProcessed processedFile = NULL;

        LdrProcessPe(pBufImageFile, processedFile);
